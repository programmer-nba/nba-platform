                    ...
                    <ion-row>
                        <ion-col v-for="(item, index) in services" :key="index" size="12">
                            <ion-card>
                                ...
                                <p style="padding-left: 5%;" v-for="detail in item.detail.split('\r\n')">
                                    {{ detail }}
                                </p>
...
export default defineComponent({
            ...
            handler: function (newQuery) {
                if (newQuery === 'confirmed') {
                    loadingController.create({
                        ...
                        a.present().then(() => {
                            this.userservice.PostWebSite(this.confirm).then((result: any) => {
                                console.log(result)
    ...
    async mounted() {
        ...
        loadingController.create({
            ...
            a.present().then(() => {
                this.userservice.GetWebSiteService().then((result: any) => {
                    console.log(result);
